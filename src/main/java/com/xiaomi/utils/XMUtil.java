...
public class XMUtil {
    ...
    public static Logger logger = LoggerFactory.getLogger(XMUtil.class.getName());
    ...
    public static String getMacAccessTokenSignatureString(String nonce, 
        ...
        logger.debug(String.format("mac sign macKey %s macAlgorithm %s", macKey, macAlgorithm));
        exps.add(nonce);
        logger.debug(String.format("mac sign nonce %s", nonce));
        exps.add(method.toUpperCase());
        logger.debug(String.format("mac sign method %s", method.toUpperCase()));
        exps.add(host);
        logger.debug(String.format("mac sign host %s", host));

        exps.add(uriPath);
        logger.debug(String.format("mac sign uriPath %s", uriPath));

        // qs可以为空
        StringBuffer sb = new StringBuffer();
        if (StringUtils.isNotEmpty(qs)) {
            logger.debug(String.format("mac sign qs %s", qs));
        ...
        // 补上最后一个\n
        joined += "\n";
        logger.debug(String.format("Mac sign string %s", joined));
     ...
     * @throws java.io.UnsupportedEncodingException
     */
    public static Map<String,String> buildMacRequestHead(String accessTokenId,
                                             String nonce, 
